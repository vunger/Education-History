<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="PDF_VERSION" select="'MacN1947.pdf'"/>
    <xsl:variable name="NO_OF_LEADING_PAGES" select="3"/>

    <!-- Global constants for navigation links -->
    <xsl:variable name="NEXT" select="'Next'"/>
    <xsl:variable name="PREV" select="'Previous'"/>
            ...
            <xsl:when test="ancestor-or-self::div1">
                <div id="bookpage">
                    ...
                    <div class="navigation">
                        <xsl:call-template name="makePrevLink"/>
                        <xsl:call-template name="makeNextLink"/>
                    </div>
                </div>
            </xsl:when>
    ...
    <!-- *** Navigation Links *** -->
    <xsl:template name="makePrevLink">
        <!-- Link to previous chapter -->
        <xsl:variable name="prevdiv" select="preceding-sibling::div1[1]"/>
        <div class="prev-link">
            <xsl:choose>
                <xsl:when test="boolean(string-length($prevdiv))">
                    <!-- previous chapter is inside same parent -->
                    <xsl:variable name="prev_ch_prefix"
                        select="substring(
                        $prevdiv/@type, 1, $TRUNC_CHAPTER_NAME)"/>
                    <xsl:variable name="prev_ch_num" select="$prevdiv/@n"/>
                    <xsl:variable name="prevfile"
                        select="concat(
                        $FILE_PREFIX, $prev_ch_prefix, $prev_ch_num, $FILE_EXT)"/>
                    <a href="{$prevfile}">
                        <xsl:value-of select="$PREV"/>
                    </a>
                </xsl:when>
                <xsl:when test="parent::body">
                    <!-- link to last chapter of front matter -->
                    <xsl:variable name="last_ch_prefix"
                        select="substring(
                        //front/div1[last()]/@type, 1, $TRUNC_CHAPTER_NAME)"/>
                    <xsl:variable name="prevfile"
                        select="concat(
                        $FILE_PREFIX, $last_ch_prefix, $FILE_EXT)"/>
                    <a href="{$prevfile}">
                        <xsl:value-of select="$PREV"/>
                    </a>
                </xsl:when>
                <xsl:when test="parent::back">
                    <!-- link to last chapter of book body -->
                    <xsl:variable name="lastdiv" select="//body/div1[last()]"/>
                    <xsl:variable name="last_ch_prefix"
                        select="substring(
                        $lastdiv/@type, 1, $TRUNC_CHAPTER_NAME)"/>
                    <xsl:variable name="last_ch_num" select="$lastdiv/@n"/>
                    <xsl:variable name="prevfile"
                        select="concat(
                        $FILE_PREFIX, $last_ch_prefix, $last_ch_num, $FILE_EXT)"/>
                    <a href="{$prevfile}">
                        <xsl:value-of select="$PREV"/>
                    </a>
                </xsl:when>
    ...
    <xsl:template name="makeNextLink">
        <!-- Link to next chapter -->
        <xsl:variable name="nextdiv" select="following-sibling::div1[1]"/>
        <div class="next-link">
            <xsl:choose>
                <xsl:when test="boolean(string-length($nextdiv))">
                    <!-- next chapter is inside same parent -->
                    <xsl:variable name="next_ch_prefix"
                        select="substring(
                    $nextdiv/@type, 1, $TRUNC_CHAPTER_NAME)"/>
                    <xsl:variable name="next_ch_num" select="$nextdiv/@n"/>
                    <xsl:variable name="nextfile"
                        select="concat(
                    $FILE_PREFIX, $next_ch_prefix, $next_ch_num, $FILE_EXT)"/>
                    <a href="{$nextfile}">
                        <xsl:value-of select="$NEXT"/>
                    </a>
                </xsl:when>
                <xsl:when test="parent::body">
                    <!-- link to first chapter of back matter -->
                    <xsl:variable name="first_ch_prefix"
                        select="substring(
                    //back/div1[1]/@type, 1, $TRUNC_CHAPTER_NAME)"/>
                    <xsl:variable name="nextfile"
                        select="concat(
                    $FILE_PREFIX, $first_ch_prefix, $FILE_EXT)"/>
                    <a href="{$nextfile}">
                        <xsl:value-of select="$NEXT"/>
                    </a>
                </xsl:when>
                <xsl:when test="parent::front">
                    <!-- link to first chapter of book body -->
                    <xsl:variable name="firstdiv" select="//body/div1[1]"/>
                    <xsl:variable name="first_ch_prefix"
                        select="substring(
                    $firstdiv/@type, 1, $TRUNC_CHAPTER_NAME)"/>
                    <xsl:variable name="first_ch_num" select="$firstdiv/@n"/>
                    <xsl:variable name="nextfile"
                        select="concat(
                    $FILE_PREFIX, $first_ch_prefix, $first_ch_num, $FILE_EXT)"/>
                    <a href="{$nextfile}">
                        <xsl:value-of select="$NEXT"/>
                    </a>
                </xsl:when>
        ...
        <xsl:choose>
            ...
            <xsl:when test="boolean(string-length($target_page/@n))">
                ...
                <xsl:variable name="target_file"
                    select="concat(
                    $FILE_PREFIX, $target_ch_prefix, $target_ch_num, $FILE_EXT)"/>
